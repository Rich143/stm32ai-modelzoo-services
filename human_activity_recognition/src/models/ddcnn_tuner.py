...
def update_and_check_size_conv_to_max_pool(
    ...
    k_effective = kernel_size + (kernel_size - 1) * (dilation_rate - 1)
...
def get_ddcnn(input_shape: tuple[int] = (24, 3, 1),
            num_classes: int = 4,
            ...
            global_max_or_average_pooling: str = "max",
           ):
    """
    ...
    model.add(layers.SeparableConv2D(
        layer_1_conf.num_filters,
        kernel_size=(layer_1_conf.kernel_size, 1),
        strides=(1, 1),
        dilation_rate=(layer_1_conf.dilation_rate, 1),
        padding="valid",
        activation="relu"))
    ...
    current_size = update_and_check_size_conv_to_max_pool(
        ...
        dilation_rate=layer_1_conf.dilation_rate,
    ...
    model.add(layers.BatchNormalization())
    model.add(layers.SeparableConv2D(
        layer_2_conf.num_filters,
        kernel_size=(layer_2_conf.kernel_size, 3),
        strides=(1, 1),
        dilation_rate=(layer_2_conf.dilation_rate, 1),
        padding="valid",
        activation="relu"))
    ...
    if layer_3_conf.optional_layer:
        current_size = update_and_check_size_conv_to_max_pool(
            current_size=current_size,
            kernel_size=layer_2_conf.kernel_size,
            dilation_rate=layer_2_conf.dilation_rate,
        ...
        model.add(layers.BatchNormalization())
        model.add(layers.SeparableConv2D(
            layer_3_conf.num_filters,
            kernel_size=(layer_3_conf.kernel_size, 1),
            strides=(1, 1),
            dilation_rate=(layer_3_conf.dilation_rate, 1),
            padding="valid",
            activation="relu"))
    ...
    # maxpooling
    if global_max_or_average_pooling == "max":
        model.add(
            layers.GlobalMaxPooling2D()
        )
    elif global_max_or_average_pooling == "average":
        model.add(
